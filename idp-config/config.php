...
$config = array(
     ...
     * reverse proxy).
     */
    // Pinned to an absolute URL so the IdP issues a stable entityID regardless
    // of whether the request comes from the browser (localhost:8082) or from
    // shibd inside the docker network (test-idp:8080). Without this, each
    // caller gets a different entityID and SAML validation fails.
    'baseurlpath' => (getenv('SIMPLESAMLPHP_BASEURLPATH') ?: 'http://localhost:8082/simplesaml/'),
    'certdir' => 'cert/',
    'loggingdir' => 'log/',
    'datadir' => 'data/',
